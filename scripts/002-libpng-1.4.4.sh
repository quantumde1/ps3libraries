...
if [ ! -f config.guess ]; then wget --continue https://raw.githubusercontent.com/gcc-mirror/gcc/refs/heads/master/config.guess; fi
if [ ! -f config.sub ]; then wget --continue https://raw.githubusercontent.com/gcc-mirror/gcc/refs/heads/master/config.sub; fi
